...
def _fix_mojibake(s):
    """
    Perbaiki kutip pintar (\u201c \u201d) yang ter-decode salah dari xlsx
    (karakter UTF-8 di file sumber terbaca sebagai cp1252/Windows-1252).
    Dipakai di mana pun label pertanyaan dibaca dari Deka_project_dataset_BankXYZ.xlsx,
    supaya hasilnya konsisten antara load_data.py dan app.py.
    """
    return (s.replace('\u00e2\u20ac\u0153', '\u201c')   # â€œ -> "
             .replace('\u00e2\u20ac\u009d', '\u201d')   # â€\x9d -> "
             .replace('\u00e2\u20ac', '\u201d')         # sisa â€ tanpa byte kontrol -> "
             .replace('\u00c2', ''))                    # Â liar -> dihapus
